...
# Make the "app" package importable when Alembic runs from the backend/ folder.
sys.path.append(str(Path(__file__).resolve().parents[1]))

from app.config import settings  # noqa: E402
from app.database import Base  # noqa: E402
from app.models import *  # noqa: E402,F401,F403  (imports all models onto Base.metadata)

# This is the Alembic Config object, which provides access to values
# within the .ini file in use.
config = context.config

# Inject our real database URL from settings/.env instead of hardcoding it in alembic.ini.
